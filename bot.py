...
def handle_article_pdf(update: Update, context: CallbackContext) -> int:
    if update.effective_chat.type != "private":
        update.message.reply_text("أرسل المقال في الخاص فقط.")
        return ConversationHandler.END
    ...
    try:
        ...
        update.message.reply_text(
            "⚠️ تم قبول المقال، لكن حدث خطأ أثناء نشره."
        )
        return ConversationHandler.END
...
def handle_video_clarify(update: Update, context: CallbackContext) -> int:
    ...
    # ================== عدّاد محاولات التوضيح ==================
    attempts = context.user_data.get("video_clarify_attempts", 0) + 1
    context.user_data["video_clarify_attempts"] = attempts

    user = update.effective_user
    username = user.username or user.first_name or "مستخدم"

    update.message.reply_text(
        "🔧 شكرًا للتفاصيل! جاري تحليلها وتجهيز برومبت الفيديو..."
    )
    ...
    # ================== ما زالت هناك تفاصيل ناقصة (محاولة أو محاولتان فقط) ==================
    if status == "need_more" and attempts < 2:
        questions = result.get("questions", [])

        if questions:
            msg = "لتحسين جودة الفيديو أكثر، أحتاج منك توضيح النقاط التالية:\n\n"
            for q in questions:
                msg += f"- {q}\n"
            msg += "\n✍️ أرسل الإجابة في رسالة واحدة."
        else:
            msg = (
                "لا تزال هناك بعض التفاصيل البسيطة الناقصة.\n"
                "رجاءً صف المشهد، الإضاءة، أسلوب التصوير، والمزاج العام."
            )

        update.message.reply_text(msg)
        return STATE_VIDEO_CLARIFY

    # ================== كسر الحلقة: إجبار الإكمال ==================
    if status == "need_more" and attempts >= 2:
        update.message.reply_text(
            "✅ وصلتني تفاصيل كافية، سأعتمد أفضل تصور ممكن وأبدأ إنشاء الفيديو الآن."
        )
        status = "ok"
    ...
    if status != "ok":
        update.message.reply_text(
            ...
            "رجاءً أعد المحاولة من البداية باستخدام /video.",
            reply_markup=MAIN_KEYBOARD,
        )
        return ConversationHandler.END
    ...
    if not final_prompt:
        update.message.reply_text(
            ...
            "حاول إعادة وصف فكرتك بتفاصيل أكثر.",
            reply_markup=MAIN_KEYBOARD,
        )
        return ConversationHandler.END
    ...
    update.message.reply_text(
        ...
        "📤 جاري إرسال الطلب إلى خدمة إنشاء الفيديو بالذكاء الاصطناعي..."
    )
